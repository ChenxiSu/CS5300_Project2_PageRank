...
public class BlockedReducer extends Reducer<LongWritable, Text, Text, Text>{
	...
	throws IOException, InterruptedException{
		...
		while(true){
			...
			totalResidual = updatePRValue(nextPR, previousPR, insideNodeComingEdgeList,boundaryCondition,innerIteration,graph,curBlockId);
 	...
 	*/
 	public static double updatePRValue(HashMap<Long, Double> nextPR, 
 		HashMap<Long, Double> previousPR, HashMap<Long, List<Double> > insideNodeComingEdgeList,
 		 HashMap<Long, List<Double> > boundaryCondition, long i,HashMap<Long, String> graph,
 		 long curBlockId){
 		double totalResidual = 0;
 		...
 		for(Long curNodeId : allNodesSet){
			...
			double prePR;
 			...
 			//update corresponding node in the insideNodeComingEdgeList!!! when a node PR
 			// has been changed
 			//System.out.println("Start updating values of outgoing edges!!!");
 			String edgeInfoStr = graph.get(curNodeId);

 			if(!edgeInfoStr.equals("")){
 				String[] edgeInfoArray = edgeInfoStr.split(",");
 			
 				int n = edgeInfoArray.length;	
 				double prChange = (finalCurPR - prePR)/n;
 				

 				for(String destNode : edgeInfoArray){
 					long destNodeId = Long.parseLong(destNode);
 					long destBlockID = BlockMatch.blockIDofNode(destNodeId);
 					if(destBlockID == curBlockId){
 						List<Double> tempEdgeListOfPRVals = new ArrayList<Double>();
 						tempEdgeListOfPRVals = insideNodeComingEdgeList.get(destNodeId);
 						double replaceNumber = tempEdgeListOfPRVals.get(0)+prChange;
 						tempEdgeListOfPRVals.set(0, replaceNumber);
 						insideNodeComingEdgeList.put(destNodeId,tempEdgeListOfPRVals);
